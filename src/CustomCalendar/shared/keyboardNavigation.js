const handleArrowKeys = (e, { allowVerticalArrows }) => {
  ...
  const getNthChildSafe = (element, index) =>
  ...
  const getVerticalSibling = (row) =>
    getNthChildSafe(
      row,
      Array.from(activeElement.parentElement.children).indexOf(activeElement)
    );
  ...
  const isDefaultSelectable =
    activeElement.dataset.isDefaultSelectable === "true";
